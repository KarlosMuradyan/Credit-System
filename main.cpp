...
class Request{
    ...
    public:
        void add_req(int i, std::string req_type, std::string req){
            location = i;
            type = req_type;
            request = req;
            status = 'p';
            
            std::ofstream requests_data("reqs.txt", std::ios_base::app);   //opening file for adding new info
            
            requests_data<<status<<"|";
            requests_data<<location<<"|";
            requests_data<<type<<"|";
            requests_data<<request<<std::endl;
            requests_data.close();
        }
        void status_change(bool a){
            if(a){
                status = "a";
            }
            else if(!a){
...
int read_reqs(Request requests[]){
    int i;
    std::string line;
    
    std::ifstream info ("reqs.txt");         //open a file
    if(info.is_open()){                //checking the open status of file
        i=0;
        //while (!info.eof()) { //checking if we reached to the end of a file
        while ( getline(info, line)) { //checking if we reached to the end of a file
            auto person_info_array = explode(line, '|');
            if (3 > person_info_array.size()) {
                std::cout << "No any information in the text file yet. Firstly, please add clients." << std::endl;
                break;
            }
            else{
            // string str(person_info_array[0].begin(), person_info_array[0].end());
                requests[i].add_req(stoi(person_info_array[1]), person_info_array[2], person_info_array[3]);
                if(person_info_array[0] == "a"){
                    requests[i].status_change(true);
                }
                else if(person_info_array[0] == "r"){
                    requests[i].status_change(false);
                }
                i++;
            }
        }
...
int admin()
{
    ...
    while (k!=5)
    {
        ...
        n = read(list);
        std::cout << "1.add client, 2.search, 3.delete, 4.print, 5.Requests, 6.End" << std::endl;
        std::cin >> k;
        switch (k)
        {
            ...
            case 5:{ 
                Requests all_reqs[100];
                int numb_of_reqs = read_reqs(all_reqs);
                int req_choice;
                std::cout<<"Click: 1.Print all requests, 2.Change status of a request, 3.End ";
                switch(req_choice){
                
                    case 1:{
                        for(int st=0; st<numb_of_reqs; st++){
                            std::cout<<"Person: "<< all_reqs[st].get_num()<<std::endl<<"Status: "<<all_reqs[st].get_st()<<std::endl<<"Request: "<<all_reqs[st].get_mes();
                    ...
                    case 2:
                        {
                            int number_of_ch;
                            std::string dec;
                            std::cout<<"Print the number of the request that you want to change. "<<std::endl;
                            std::cin>>number_of_ch;
                            std::cout<<"For acceptance:a , For rejection: r";
                            std::cin>>dec;
                            bool decision
                            if(dec =="a"){
                                decision = true;
                            ...
                            switch(choice){
                                ...
                                case 2:{
                                    ...
                                    if((tp == "1" || tp=="2") && num<5 ){
                                        ...
                                        new_arr[num].add_req(i, tp, message);
                                        n++;
                                    }
                                    else{
